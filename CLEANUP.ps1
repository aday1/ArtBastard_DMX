...
if (Test-Path $ViteCacheDir) {
    Write-Host "Removing Vite cache: $ViteCacheDir" -ForegroundColor DarkCyan
    Remove-Item -Recurse -Force $ViteCacheDir
}
if (Test-Path $RootEslintCache) {
    Write-Host "Removing root .eslintcache" -ForegroundColor DarkCyan
    Remove-Item -Force $RootEslintCache
}
if (Test-Path $ReactAppEslintCache) {
    Write-Host "Removing react-app .eslintcache" -ForegroundColor DarkCyan
...
if (Test-Path $RootNodeModules) {
    Write-Host "OPTIONAL: Attempting to remove root node_modules: $RootNodeModules (This is a commitment, darling!)" -ForegroundColor DarkYellow
    try {
...
if (Test-Path $ReactAppNodeModules) {
    Write-Host "OPTIONAL: Attempting to remove react-app node_modules: $ReactAppNodeModules (This is a commitment, darling!)" -ForegroundColor DarkYellow
    try {
...
if (Test-Path $LauncherNodeModules) {
    Write-Host "OPTIONAL: Attempting to remove launcher node_modules: $LauncherNodeModules" -ForegroundColor DarkYellow
    try {
